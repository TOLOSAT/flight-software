...
Between each of these tasks, buffers are used to store messages. For a given type of message between two tasks, a single buffer is used. There are several types of message that can be exchanged between tasks :
- TC message, the buffer necessarily has the TC Receiver as its source and a task capable of processing TCs as its destination.
- TM message, the buffer's destination must be the TM Sender and its source must be a task capable of processing TCs.
- Life Message, this buffer lets SALAMI know the current status of all the tasks. The destination of these buffers is necessarily SALAMI.
- Mode Change Request Message, this buffer allows certain tasks (MISO and CARNE) to warn SALAMI of an error or event requiring a mode change or even a reset.
...
Throughout its lifetime, the satellite will have several modes. Operating in different modes allows the satellite to function differently depending on the context. A distinction is made between nominal modes, which occur when the satellite is operating normally, and other modes, which occur in special circumstances. The modes and their activations are summarised in the following graph:

<center><img src="../images/Satellite_Modes_Graph.png" width=65% /></center>

The following modes are available :
- BOOT : is not really a mode, but rather the section of bootloader code that loads the right version of the software with the right initial conditions.
- LAUNCH : allows solar panels to be deployed and detumbling to be carried out after being deployed from the rocket.
- SAFE : is the mode in which the satellite's minimum functions are performed, and is intended to guarantee the satellite's safety.
- IDLE : nominal default mode, it performs more actions than SAFE mode but does not transmit data or carry out missions.
- TRANSMISSION : is triggered by TC when the satellite is above the ground station and is used to send data back down to the ground.
- MISSION GRAVIMETRY / IRIDIUM : enables the satellite to carry out its mission.
- END OF LIFE : used to deactivate and disconnect the solar panels and drain the batteries when the satellite reaches the end of its life.
