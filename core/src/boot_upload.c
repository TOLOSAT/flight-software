...
void UploadSoftware(void)
{
    ...
    for (uint32_t i = 0u; i < elf_header.e_phnum; ++i)
    {
        ...
        // Check whether this segment should be loaded into memory
...
/**
 * @fn      GetSoftwareCRC(void)
 * @brief   Get the CRC at the end of the software file
 * @return  CRC32
 */
static uint32_t GetSoftwareCRC(void)
{
    ...
    f_read(&file, &crc32, sizeof(crc32), NULL);

    // Deal with endianness
    crc32 = ((crc32 & 0x000000FFu) << 24u) | ((crc32 & 0x0000FF00u) << 8u) | ((crc32 & 0x00FF0000u) >> 8u) | ((crc32 & 0xFF0000FFu) >> 24u);
...
/**
 * @fn      ComputeSoftwareCRC(void)
 * @brief   Compute the CRC reading the file
 * @return  CRC32
 */
static uint32_t ComputeSoftwareCRC(void)
{
    ...
    // Lire le fichier par blocs et calculer le CRC
    while (remaining > 0u)
    {
        // Update Chunk Size according to the remaining data number
        if (remaining < BUFFER_SIZE)
        ...
        // Compute CRC of the read chunk
        for (uint32_t i = 0u; i < chunk_size; i++)
        {
            crc32 = g_crc32_lookup_table[(crc32 ^ buffer[i]) & 0xFFu] ^ (crc32 >> 8);
        }

        // Update remaining byte variable
        remaining -= chunk_size;
    }
