 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION SendEventMessage(eventMessage_t *event_message, bufferRef_t buffer_ref)
{
    ...
    {
        coreStatus_t test_val = WriteBuffer(buffer_ref, event_message, EVENT_MESSAGE_SIZE);
 ...
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION ReceiveEventMessage(eventMessage_t *event_message, bufferRef_t buffer_ref)
{
    // Variable Initialisation
    coreStatus_t return_val = CORE_SUCCESSFUL;
    ...
    if (event_message != NULL)
    {
        coreStatus_t test_val = ReadBuffer(buffer_ref, event_message, EVENT_MESSAGE_SIZE);
        if (test_val != CORE_SUCCESSFUL)
        {
