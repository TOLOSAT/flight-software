 ...
 */
static timerInst_t IN_CORE_DATA_SECTION hal_tick_timer;

/**
 * @var     monitoring_timer
 * @brief   Monitoring timer instance used for FreeRTOS monitoring
 */
static timerInst_t IN_CORE_DATA_SECTION monitoring_timer;

/**
 * @var     monitoring_tick
 * @brief   Tick for freertos monitoring
 */
static volatile uint64_t IN_CORE_DATA_SECTION monitoring_tick;
 ...
 */
void IN_CORE_TEXT_SECTION HalDelay(uint32_t delay)
{
    HAL_Delay(delay);
}
 ...
 * @warning Do not use this function inside a thread, please prefer the OS API
 */
uint32_t IN_CORE_TEXT_SECTION HalGetTick(void)
{
    return HAL_GetTick();
}
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef IN_CORE_TEXT_SECTION HAL_InitTick(uint32_t TickPriority)
{
    ...
    */
    ...
    if (status == HAL_OK)
    {
        ...
        coreStatus_t test_irq = RequestIRQ(TIM4_IRQn, TickPriority, &HalTickHandler, NULL);
        if (test_irq == CORE_SUCCESSFUL)
        {
 ...
 */
void IN_CORE_TEXT_SECTION HAL_SuspendTick(void)
{
 ...
 */
void IN_CORE_TEXT_SECTION HAL_ResumeTick(void)
{
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION InitMonitoringTimer(void)
{
 ...
 */
void IN_CORE_TEXT_SECTION StartMonitoringTimer(void)
{
 ...
 */
uint64_t IN_CORE_TEXT_SECTION GetMonitoringTick(void)
{
 ...
 */
static void IN_CORE_TEXT_SECTION MonitoringTickHandler(void *param)
{
    // Unused Parameter
    (void)(param);
 ...
 */
static void IN_CORE_TEXT_SECTION HalTickHandler(void *param)
{
    // Unused Parameter
    (void)(param);
