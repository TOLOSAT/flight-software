...
static coreStatus_t SpiSetupIRQs(spiInst_t *spi_inst);
 ...
 * @retval          #CORE_INVALID_PARAM if SPI ref is not available for this board or one pointer is null
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiOpen(spiInst_t *spi_inst)
{
    ...
    if (spi_inst != NULL)
    {
        ...
        if (return_value != CORE_INVALID_PARAM)
        {
 ...
 * @retval      #CORE_TIMEOUT if spi timed out before sending message
 * @retval      #CORE_BUSY if spi is still sending previous message
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiWrite(spiInst_t *spi_inst, spiMsg_t *msg, spiMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_SUCCESSFUL if message sent successfully
 * @retval      #CORE_INVALID_PARAM if one pointer is null
 * @retval      #CORE_TIMEOUT if spi timed out before receiving message
 * @retval      #CORE_BUSY if spi is still receiving previous message
 * @retval      #CORE_ERROR if transmit went wrong
 *
 * Attention : currently works only in polling and interrupt mode
 * Needs to supports DMA
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiRead(spiInst_t *spi_inst, spiMsg_t *received_msg, spiMsg_t *transmit_msg, spiMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiIoctl(spiInst_t *spi_inst, uint32_t cmd, void *data, uint32_t data_size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiClose(spiInst_t *spi_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]   spi_inst Instance that contains SPI parameters and SPI Handler
 * @retval      #CORE_SUCCESSFUL if changing parameters succeed
 * @retval      #CORE_INVALID_PARAM if IT is not available for this SPI
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSetupIRQs(spiInst_t *spi_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
static void IN_CORE_TEXT_SECTION SpiGenericIRQHandler(void *param)
{
