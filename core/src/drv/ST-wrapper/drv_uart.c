...
static coreStatus_t UartSetUpDMA(uartInst_t *uart_inst);
static coreStatus_t UartSetupIRQs(uartInst_t *uart_inst);
static coreStatus_t UartDMAorITStartRX(uartInst_t *uart_inst, void *data, uint32_t data_size);
static coreStatus_t UartDMAorITStartTX(uartInst_t *uart_inst, void *data, uint32_t data_size);
static coreStatus_t UartDMAorITCheckRXEnded(uartInst_t *uart_inst, void *data, uint32_t data_size);
static coreStatus_t UartDMAorITCheckTXEnded(uartInst_t *uart_inst, void *data, uint32_t data_size);
 ...
 * @retval  #CORE_INVALID_PARAM if UART ref is not available for this board, baudrate or one pointer is null
 */
coreStatus_t IN_CORE_TEXT_SECTION UartOpen(uartInst_t *uart_inst)
{
 ...
 * @retval      #CORE_SUCCESSFUL if message sent successfully
 * @retval      #CORE_INVALID_PARAM if one pointer is null
 * @retval      #CORE_TIMEOUT if uart timed out before sending message
 * @retval      #CORE_BUSY if uart is still sending previous message
 * @retval      #CORE_ERROR if transmit went wrong
 */
coreStatus_t IN_CORE_TEXT_SECTION UartWrite(uartInst_t *uart_inst, uartMsg_t *msg, uartMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_ERROR if transmit went wrong
 */
coreStatus_t IN_CORE_TEXT_SECTION UartRead(uartInst_t *uart_inst, uartMsg_t *msg, uartMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval          #CORE_INVALID_PARAM if instance is a null pointer
 * @retval          #CORE_BUSY if action cannot be performed because driver is busy
 * @retval          #CORE_ERROR if io control encountered an error
 * @retval          #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION UartIoctl(uartInst_t *uart_inst, uint32_t cmd, void *data, uint32_t data_size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in,out]   uart_inst Instance that contains UART parameters and UART Handler
 * @retval          #CORE_SUCCESSFUL if changing parameters succeed
 * @retval          #CORE_INVALID_PARAM if instance is a null pointer
 *
 * This function erase uart_inst
 */
coreStatus_t IN_CORE_TEXT_SECTION UartClose(uartInst_t *uart_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]   uart_inst Instance that contains UART parameters and UART Handler
 * @retval      #CORE_SUCCESSFUL if changing parameters succeed
 * @retval      #CORE_INVALID_PARAM if DMA is not available for this UART
 */
static coreStatus_t IN_CORE_TEXT_SECTION UartSetUpDMA(uartInst_t *uart_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]   uart_inst Instance that contains UART parameters and UART Handler
 * @retval      #CORE_SUCCESSFUL if changing parameters succeed
 * @retval      #CORE_INVALID_PARAM if IT is not available for this UART
 */
static coreStatus_t IN_CORE_TEXT_SECTION UartSetupIRQs(uartInst_t *uart_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval          #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION UartDMAorITStartRX(uartInst_t *uart_inst, void *data, uint32_t data_size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval          #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION UartDMAorITStartTX(uartInst_t *uart_inst, void *data, uint32_t data_size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]       data_size Data size
 * @retval          #CORE_INVALID_PARAM if instance is a null pointer
 * @retval          #CORE_BUSY if DMA is still receiving data
 * @retval          #CORE_ERROR if io control encountered an error
 * @retval          #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION UartDMAorITCheckRXEnded(uartInst_t *uart_inst, void *data, uint32_t data_size)
{
    // Unused Parameters
    (void)(data);
    (void)(data_size);
 ...
 * @param[in]       data_size Data size
 * @retval          #CORE_INVALID_PARAM if instance is a null pointer
 * @retval          #CORE_BUSY if DMA is still transfering data
 * @retval          #CORE_ERROR if io control encountered an error
 * @retval          #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION UartDMAorITCheckTXEnded(uartInst_t *uart_inst, void *data, uint32_t data_size)
{
    // Unused Parameters
    (void)(data);
    (void)(data_size);
 ...
 */
static void IN_CORE_TEXT_SECTION UartGenericIRQHandler(void *param)
{
 ...
 */
static void IN_CORE_TEXT_SECTION UartGenericDMAIRQHandler(void *param)
{
