...
static coreStatus_t GpioToggle(gpioInst_t *gpio_inst);
static void GpioGenericIRQHandler(void *param);
static coreStatus_t GpioSetupIRQs(gpioInst_t *gpio_inst);
 ...
 * @retval          #CORE_INVALID_PARAM if GPIO port is not available for this board, pin = 0 or one pointer is null
 *
 * Attention : GPIO_PIN_0 != 0, GPIO_PIN_0=0x0001 (cf drv_gpio.h)
 */
coreStatus_t IN_CORE_TEXT_SECTION GpioOpen(gpioInst_t *gpio_inst)
{
 ...
 * @retval      #CORE_SUCCESSFUL if write succeed
 * @retval      #CORE_INVALID_PARAM if GPIO is not an output or instance is a null pointer
 */
coreStatus_t IN_CORE_TEXT_SECTION GpioWrite(gpioInst_t *gpio_inst, gpioValue_t value)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION GpioRead(gpioInst_t *gpio_inst, gpioValue_t *value)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION GpioIoctl(gpioInst_t *gpio_inst, uint32_t cmd, void *data, uint32_t data_size)
{
    // Unused
    (void)(data);
    (void)(data_size);
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION GpioClose(gpioInst_t *gpio_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]   gpio_inst Instance that contains GPIOs parameters
 * @retval      #CORE_SUCCESSFUL if toggle succeed
 * @retval      #CORE_INVALID_PARAM if GPIO is not an output or instance is a null pointer
 */
static coreStatus_t IN_CORE_TEXT_SECTION GpioToggle(gpioInst_t *gpio_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]   gpio_inst Instance that contains GPIOs parameters
 * @retval      #CORE_SUCCESSFUL if changing parameters succeed
 * @retval      #CORE_INVALID_PARAM if IT is not available for this GPIO
 */
static coreStatus_t IN_CORE_TEXT_SECTION GpioSetupIRQs(gpioInst_t *gpio_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
static void IN_CORE_TEXT_SECTION GpioGenericIRQHandler(void *param)
{
