...
static coreStatus_t SpiSD_InitHw(void);
static coreStatus_t SpiSD_Select(void);
static coreStatus_t SpiSD_Unselect(void);
static coreStatus_t SpiSD_WaitUntilReady(void);
static coreStatus_t SpiSD_SwitchOn(void);
static coreStatus_t SpiSD_SwitchOff(void);
static coreStatus_t SpiSD_RxDataBlock(uint8_t *buff, uint32_t len);
static coreStatus_t SpiSD_TxDataBlock(const uint8_t *buff, uint32_t len, uint8_t token);
static coreStatus_t SpiSD_SendCmd(uint8_t cmd, uint32_t arg, uint8_t *answer, uint32_t answer_size);
static coreStatus_t SpiSD_SendBytes(uint8_t *data, uint32_t size);
static coreStatus_t SpiSD_ReceiveBytes(uint8_t *data, uint32_t size);
static uint8_t ComputeCommandCRC7(const uint8_t *cmd_msg);

/*************************** Variables Definitions ***************************/

static DSTATUS IN_CORE_DATA_SECTION g_disk0_status = STA_NOINIT; /**< Disk0 Status */
SDCardStatus_t IN_CORE_DATA_SECTION g_sd_card_status = SD_CARD_OFF; /**< Indicates if SD card is ON/OFF */
SDCardType_t IN_CORE_DATA_SECTION g_sd_card_type = NOT_SDCARD;      /**< SD card type */

/**
 * @var     sd_card_gpio
 * @brief   GPIO for sd card (cs or card detect depend of the context) instance declaration
 */
static gpioInst_t IN_CORE_DATA_SECTION sd_card_gpio = {
    .port = SD_PORT,
    .pin = SD_GPIO_PIN,
 ...
 */
static spiInst_t IN_CORE_DATA_SECTION spi_sd_card_inst = {
 ...
 * @return      DSTATUS 
 */
DSTATUS IN_CORE_TEXT_SECTION SpiSD_DiskStatus(uint8_t disk)
{
 ...
 * @retval      #CORE_INVALID_PARAM if disk does not exist
 * @retval      #CORE_ERROR if initialisation failed
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiSD_DiskInit(uint8_t disk)
{
    // Variables Initialization
    ...
    if (return_value == CORE_SUCCESSFUL)
    {
        ...
        if (disk == DISK0_REF)
        {
            ...
            coreStatus_t test_hal = SpiSD_SwitchOn();
            if (test_hal == CORE_SUCCESSFUL)
            {
                ...
                if (test_hal == CORE_SUCCESSFUL)
                {
                    ...
                    if (test_hal == CORE_SUCCESSFUL)
                    {
                        ...
                        if ((interface_condition[2] == (uint8_t)((0x0000ff00u & SD_CARD_INTERFACE_COND) >> 8u)) && (interface_condition[3] == (uint8_t)(0x000000ffu & SD_CARD_INTERFACE_COND)))
                        {
                            ...
                            test_hal = CORE_TIMEOUT;
                            while ((test_hal != CORE_SUCCESSFUL) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
                            {
 ...
 * @retval      #CORE_INVALID_PARAM if disk does not exist, len equal zero, pointer is null
 * @retval      #CORE_TIMEOUT if disk is not available
 * @retval      #CORE_ERROR if an error occured
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiSD_DiskRead(uint8_t disk, uint8_t *data, uint32_t addr, uint32_t len)
{
    // Variables Initialization
    coreStatus_t return_value = CORE_SUCCESSFUL;
    ...
    if ((disk == DISK0_REF) && (len != 0u) && (data != NULL))
    {
        ...
        else
        {
            ...
            coreStatus_t test_val = CORE_SUCCESSFUL;
            if (len == 1u)
            {
                /* READ_SINGLE_BLOCK */
            ...
            else
            {
                ...
                if (test_val == CORE_SUCCESSFUL)
                {
                    while ((sector_read < len) && (test_val == CORE_SUCCESSFUL))
                    {
 ...
 * @retval      #CORE_INVALID_PARAM if disk does not exist, len equal zero, pointer is null
 * @retval      #CORE_TIMEOUT if disk is not available
 * @retval      #CORE_ERROR if an error occured or write is not permitted
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiSD_DiskWrite(uint8_t disk, const uint8_t *data, uint32_t addr, uint32_t len)
{
        // Variables Initialization
    coreStatus_t return_value = CORE_SUCCESSFUL;
    DWORD sector_address = addr;
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (test_val == CORE_SUCCESSFUL)
                    {
                        while ((sector_written < len) && (test_val == CORE_SUCCESSFUL))
                        {
 ...
 * @retval          #CORE_INVALID_PARAM if the io control is not available for this device 
 * @retval          #CORE_ERROR if an error occured 
 * @retval          #CORE_SUCCESSFUL else 
 */
coreStatus_t IN_CORE_TEXT_SECTION SpiSD_DiskIoctl(uint8_t disk, uint8_t cmd, void *data)
{
    // Variables Initialization
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t test_hal = CORE_SUCCESSFUL;
 ...
 * @retval  #CORE_ERROR if SPI or GPIO are not initialised
 * @retval  #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_InitHw(void)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t hal_status = CORE_SUCCESSFUL;

    // Function Core
    hal_status = SpiOpen(&spi_sd_card_inst);
    if (hal_status == CORE_SUCCESSFUL)
    {
 ...
 * @retval  #CORE_ERROR if SPI or GPIO error occured
 * @retval  #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_Select(void)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t test_hal = CORE_SUCCESSFUL;
 ...
 * @retval  #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_Unselect(void)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t test_hal = CORE_SUCCESSFUL;
 ...
 * @retval  #CORE_SUCCESSFUL if SD card is ready (spi slave register is now empty)
 * @retval  #CORE_TIMEOUT if function timeouted before clearing SD card being ready
 * @retval  #CORE_ERROR if SPI has encountered an error
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_WaitUntilReady(void)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t test_hal = CORE_SUCCESSFUL;
    ...
    while ((test_hal == CORE_SUCCESSFUL) && (answer != SPI_FILL_CHAR) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
    {
 ...
 * @retval  #CORE_ERROR if SPI has encountered an error
 * @retval  #CORE_TIMEOUT if SD card never answered IDLE state
 * @retval  #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_SwitchOn(void)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t test_hal = CORE_SUCCESSFUL;
    ...
    if (test_hal == CORE_SUCCESSFUL)
    {
        ...
        {
            // Wait until SD card
            while ((test_hal == CORE_SUCCESSFUL) && (answer != SD_IDLE_FLAG) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
            {
                test_hal = SpiSD_ReceiveBytes(&answer, 1u);
            }
            ...
            if ((test_hal == CORE_SUCCESSFUL) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
            {
 ...
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_SwitchOff(void)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_INVALID_PARAM if buff is null pointer or len is null
 * @retval      #CORE_ERROR if SPI has encountered an error
 * @retval      #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_RxDataBlock(uint8_t *buff, uint32_t len)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t test_hal = CORE_SUCCESSFUL;
    ...
    if ((buff != NULL) && (len != 0u))
    {
        ...
        uint32_t tickstart = HAL_GetTick();
        while ((test_hal == CORE_SUCCESSFUL) && (token == SPI_FILL_CHAR) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
        {
            test_hal = SpiSD_ReceiveBytes(&token, 1u);
        }

        // Check if read was successful and gets a start block token
        if ((token == SD_START_BLOCK_TOKEN) && (test_hal == CORE_SUCCESSFUL) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
        {
            // Receive block
 ...
 * @retval      #CORE_INVALID_PARAM if buff is null pointer or len is null except if token is SD_STOP_TOKEN
 * @retval      #CORE_ERROR if SPI has encountered an error
 * @retval      #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_TxDataBlock(const uint8_t *buff, uint32_t len, uint8_t token)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    ...
    else
    {
        ...
        coreStatus_t test_wait = CORE_SUCCESSFUL;
        test_wait = SpiSD_WaitUntilReady();
        if (test_wait == CORE_SUCCESSFUL)
        {
            // Send token
            coreStatus_t test_hal = CORE_SUCCESSFUL;
                ...
                if (token != SD_STOP_TOKEN)
                {
                    ...
                    if (test_hal == CORE_SUCCESSFUL)
                    {
                        ...
                        if (test_hal == CORE_SUCCESSFUL)
                        {
                            ...
                            if ((test_hal == CORE_SUCCESSFUL) && (answer != SPI_FILL_CHAR) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
                            {
 ...
 * @retval      #CORE_INVALID_PARAM if command is invalid, or answer is null pointer but answer_size non null
 * @retval      #CORE_TIMEOUT if SD card was not ready or CMD12 still busy
 * @retval      #CORE_ERROR if an error occured
 * @retval      #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_SendCmd(uint8_t cmd, uint32_t arg, uint8_t *answer, uint32_t answer_size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    coreStatus_t test_wait;
    coreStatus_t test_hal;
        ...
        if ((cmd >= 0x40u) && (cmd <= 0x7fu))
        {
            ...
            if (test_wait == CORE_SUCCESSFUL)
            {
                ...
                if (test_hal == CORE_SUCCESSFUL)
                {
                    ...
                    while ((command_status == SPI_FILL_CHAR) && (test_hal == CORE_SUCCESSFUL) && ((HAL_GetTick() - tickstart) <  SD_TIMEOUT))
                    {
                        test_hal = SpiSD_ReceiveBytes(&command_status, 1u);
                    }

                    // Check Result
                    if ((test_hal == CORE_SUCCESSFUL) && (command_status <= SD_IDLE_FLAG))
                    {
 ...
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_SendBytes(uint8_t *data, uint32_t size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    uint32_t i = 0u;
    ...
    // Function Core
    while ((return_value == CORE_SUCCESSFUL) && (i < size))
    {
 ...
 */
static coreStatus_t IN_CORE_TEXT_SECTION SpiSD_ReceiveBytes(uint8_t *data, uint32_t size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    uint8_t fill_char = SPI_FILL_CHAR;
 ...
 */
static uint8_t IN_CORE_TEXT_SECTION ComputeCommandCRC7(const uint8_t *cmd_msg)
{
    // Variable Initialisation
