...
static uint32_t IN_CORE_DATA_SECTION *ramfs_ptr = &__ramfs_start__;
static DSTATUS IN_CORE_DATA_SECTION disk_stat = STA_NOINIT;
 ...
 * @return      DSTATUS
 */
DSTATUS IN_CORE_TEXT_SECTION RAM_DiskStatus(uint8_t disk)
{
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION RAM_DiskInit(uint8_t disk)
{
 ...
 * @retval      #CORE_BUSY if disk is not available
 * @retval      #CORE_ERROR if an error occured
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION RAM_DiskRead(uint8_t disk, uint8_t *data, uint32_t addr, uint32_t len)
{
    // Variables Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION RAM_DiskWrite(uint8_t disk, const uint8_t *data, uint32_t addr, uint32_t len)
{
    // Variables Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval          #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION RAM_DiskIoctl(uint8_t disk, uint8_t cmd, void *data)
{
    // Variables Initialization
    coreStatus_t return_value = CORE_SUCCESSFUL;
