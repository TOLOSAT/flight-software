...
static coreStatus_t OwWriteByte(owInst_t *ow_inst, uint8_t byte);
static coreStatus_t OwReadByte(owInst_t *ow_inst, uint8_t *byte);
static coreStatus_t OwInitConnection(owInst_t *ow_inst);
static coreStatus_t OwWriteBit(owInst_t *ow_inst, uint8_t bit);
static coreStatus_t OwReadBit(owInst_t *ow_inst, uint8_t *bit);
static coreStatus_t OwTimerInit(owInst_t *ow_inst);
 ...
 * @retval          #CORE_INVALID_PARAM if ow_inst is a null pointer
 * @retval          #CORE_ERROR if an error occured
 * @retval          #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION OwOpen(owInst_t *ow_inst)
{
 ...
 * @retval      #CORE_INVALID_PARAM if there is a null pointer or length is zero
 * @retval      #CORE_ERROR if an error occured when using GPIO
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION OwWrite(owInst_t *ow_inst, owMsg_t *msg, owMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    ...
    {
        uint32_t i = 0u;
        while ((return_value == CORE_SUCCESSFUL) && (i < length))
        {
 ...
 * @retval      #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION OwRead(owInst_t *ow_inst, owMsg_t *msg, owMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval          #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION OwIoctl(owInst_t *ow_inst, uint32_t cmd, void *data, uint32_t data_size)
{
    // Unused
    (void)(data);
    (void)(data_size);
 ...
 * @retval          #CORE_SUCCESSFUL else
 */
coreStatus_t IN_CORE_TEXT_SECTION OwClose(owInst_t *ow_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_INVALID_PARAM if there is a null pointer
 * @retval      #CORE_ERROR if an error occured when using GPIO
 * @retval      #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION OwWriteByte(owInst_t *ow_inst, uint8_t byte)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
    ...
    {
        uint32_t i = 0u;
        while ((return_value == CORE_SUCCESSFUL) && (i < 8u))
        {
 ...
 * @retval      #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION OwReadByte(owInst_t *ow_inst, uint8_t *byte)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in,out]   ow_inst Instance that contains One Wire parameters handlers
 * @retval          #CORE_INVALID_PARAM if ow_inst is a null pointer
 * @retval          #CORE_BUSY line is busy, somebody is pulling the line low
 * @retval          #CORE_BUSY if nobody has answered the master after a reset pulse
 * @retval          #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION OwInitConnection(owInst_t *ow_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION OwWriteBit(owInst_t *ow_inst, uint8_t bit)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_SUCCESSFUL else
 */
static coreStatus_t IN_CORE_TEXT_SECTION OwReadBit(owInst_t *ow_inst, uint8_t *bit)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval          #CORE_ERROR if timer has encountered an error at init
 * @retval          #CORE_INVALID_PARAM if there is a null pointer
 * @retval          #CORE_SUCCESSFUL else 
 */
static coreStatus_t IN_CORE_TEXT_SECTION OwTimerInit(owInst_t *ow_inst) 
{
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]   delay_us 
 */
static void IN_CORE_TEXT_SECTION OwDelayUs(owInst_t *ow_inst, uint32_t delay_us)
{
    if (ow_inst != NULL)
    {
