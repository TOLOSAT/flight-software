...
static coreStatus_t I2cSetupIRQs(i2cInst_t *i2c_inst);
 ...
 * @retval          #CORE_INVALID_PARAM if I2C ref is not available for this board or one pointer is null
 */
coreStatus_t IN_CORE_TEXT_SECTION I2cOpen(i2cInst_t *i2c_inst)
{
 ...
 * @retval      #CORE_TIMEOUT if i2c timed out before sending message
 * @retval      #CORE_BUSY if i2c is still sending previous message
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION I2cWrite(i2cInst_t *i2c_inst, i2cSlaveAddr_t slave_addr, i2cMsg_t *msg, i2cMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @retval      #CORE_SUCCESSFUL if message sent successfully
 * @retval      #CORE_INVALID_PARAM if one pointer is null
 * @retval      #CORE_TIMEOUT if i2c timed out before receiving message
 * @retval      #CORE_BUSY if i2c is still receiving previous message
 * @retval      #CORE_ERROR if transmit went wrong
 *
 * Attention : currently works only in polling and interrupt mode
 * Needs to supports DMA
 */
coreStatus_t IN_CORE_TEXT_SECTION I2cRead(i2cInst_t *i2c_inst, i2cSlaveAddr_t slave_addr, i2cMsg_t *msg, i2cMsgLength_t length)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION I2cIoctl(i2cInst_t *i2c_inst, uint32_t cmd, void *data, uint32_t data_size)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
coreStatus_t IN_CORE_TEXT_SECTION I2cClose(i2cInst_t *i2c_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 * @param[in]   i2c_inst Instance that contains I2C parameters and I2C Handler
 * @retval      #CORE_SUCCESSFUL if changing parameters succeed
 * @retval      #CORE_INVALID_PARAM if IT is not available for this I2C
 */
static coreStatus_t IN_CORE_TEXT_SECTION I2cSetupIRQs(i2cInst_t *i2c_inst)
{
    // Variable Initialisation
    coreStatus_t return_value = CORE_SUCCESSFUL;
 ...
 */
static void IN_CORE_TEXT_SECTION I2cGenericIRQHandler(void *param)
{
