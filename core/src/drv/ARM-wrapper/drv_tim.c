 ...
 */
static IN_CORE_DATA_SECTION DUALTIM_HandleTypeDef haltick_timer = {0};
 ...
 */
HAL_StatusTypeDef IN_CORE_TEXT_SECTION HAL_InitTick(void)
{
    ...
    if (hal_status == HAL_OK)
    {
        ...
        coreStatus_t irq_status = RequestIRQ(DUALTIMER_IRQn, HAL_TIMER_IRQ_PRIO, &HalTickHandler, NULL);
        if (irq_status == CORE_SUCCESSFUL)
        {
            // Start the timer
 ...
 */
static void IN_CORE_TEXT_SECTION HalTickCallback(DUALTIM_TimerSelTypeDef sel)
{
